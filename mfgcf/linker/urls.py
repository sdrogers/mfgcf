...
urlpatterns = [
    ...
    url(
        r'^get_graph/(?P<analysis_id>\w+)/(?P<metabanalysis_id>\w+)/(?P<families>\w+)/(?P<link_threshold>\d+(?:\.\d+))$',
    ...
    url(r'^uploadAntiSmash/$', views.UploadAntiSmash.as_view(), name="upload_anti_smash"),
    url(r'^uploadAntiSmashHandler/?$',
        views.UploadAntiSmashHandler.as_view(), name='upload_anti_smash_handler'),
    url(r'^uploadAntiSmashComplete/?$',
        views.UploadAntiSmashComplete.as_view(),
        name='upload_anti_smash_complete'),
]
